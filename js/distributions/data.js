...

    {
        ...
        distributions: [{
            ...
            },
            {
                ...
                name: "Neuniforma",
                info: "O variabila ce ia valori din multimea \\({1, 2, ..., n}\\), valorile putand avea probabilitati diferite. O multime de alt tip poate fi simulata considerand ca valorile obtinute de aceasta simulare sunt indicii elementelor din multimea reala.",
                _mean: "\\(\\sum_{k=1}^n kp_k\\)",
                _variation: "\\(\\sum_{k=1}^n(M[X]-k)^2p_k\\)",
                parameters: [{
                    label: "Numarul de valori",
                    constraint: (n) => {
                        return n > 0 && !isNaN(n);
                    ...
                    label: "Probabilitati, separate prin virgula(numere reale, punctul este separator de zecimale. Accepta inclusiv fractii de tip a/b)",
                    constraint: (p) => {
                        return nonUniform.normalize(p).good;
                    },
                    input: {
                        type: "text",
                        placeholder: "probabilitati",
                    },
                    normalize: (p) => {
                        return nonUniform.normalize(p).array;
                    }
                }],
                simulate: nonUniform.Simulate,
                analyze: nonUniform.Analyze,
                mean: nonUniform.mean,
                variation: nonUniform.variation,
                yCoords: nonUniform.yCoords,
                vData: nonUniform.vData,
            }
        ]
    }, {
