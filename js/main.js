let choice1 = q(".choice-1"),
    choice2 = _q(".choice-2"),
    dataInput = q(".data-input"),
    choices = q(".choices");
...
let vars = _q(".choice-1 .options span"),
    dist = _q(".choice-2");
...
let distr = _q(".choice-2 .options span"),
    name = q(".name"),
    desc = q(".desc"),
    inputs = q(".inputs"),
    currD, type = q(".type");
let f_mean = q(".formula-mean>span"),
    f_variation = q(".formula-variation>span");
...
let form = q("form"),
    good, mean = q(".mean>span"),
    tmean = q(".tmean>span"),
    variation = q(".variation>span"),
    tvariation = q(".tvariation>span");
let results = q(".results");
let valoriInput = q(".valori input"),
    aparitii = q(".val-ap>span"),
    prob = q(".val-prob>span"),
    teo_prob = q(".val-prob-teo>span");
...
l("submit", form, (e) => {
    ...
    if (good) {
        ...
        q(".back2").classList.remove("hidden");
        yC = currD.yCoords;
...
l("click", back2, () => {
    ...
    _q("form input").forEach(e => {
        if (e.getAttribute("type") != "submit") e.value = "";
    })
    q("[placeholder='valoare']").value = "";
    aparitii.innerHTML = "";
    prob.innerHTML = "";
    teo_prob.innerHTML = "";
    _q("canvas").forEach(e => {
...
let plot_menu = _q(".plot-menu>span"),
    plots = _q(".plot-data>span");
